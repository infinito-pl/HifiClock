...
# Czcionki (Barlow z katalogu assets/fonts/)
...
font_large = pygame.font.Font(font_bold, 212)   # godzina pogrubiona i większa
...
# Załaduj ikonę pogody (tymczasowo jako prostokąt)
weather_icon = pygame.Surface((62, 48))
weather_icon.fill(WHITE)  # zamień na załadowane SVG/PNG gdy będzie gotowe

icon_cache = {}

# Funkcja do ładowania ikon pogody
...
# Funkcja do pobierania lokalizacji po IP
...
# Funkcja do pobierania pogody z OpenWeatherMap
...
# Inicjalizacja pogody (odświeżana co 15 minut)
API_KEY = "3ca806c1d8f158812419bec229533068"
last_weather_check = 0
weather_text = "Loading weather..."
city = get_city_from_ip()

print("Wykryte miasto:", city)

test_url = f"https://api.openweathermap.org/data/2.5/weather?q={city}&appid={API_KEY}&units=metric"
...
# Zmienne pomocnicze
prev_time_text = ""
fade_progress = 1.0  # 1.0 = brak animacji
fade_start_time = 0
fade_duration = 1.0  # dłuższe przejście
prev_second = -1
prev_ring_surface = None
current_ring_surface = None

# Funkcja do rysowania tła falowego
def draw_wave_background(surface, time_offset, night_mode=False):
    if night_mode:
        surface.fill((20, 0, 0))  # ciemna czerwień
        return

    surface.fill((0, 0, 0))  # czarne tło bazowe
    colors = [
        (45, 105, 140),   # brighter steel blue
        (80, 0, 160),     # brighter indigo
        (160, 40, 40),    # brighter firebrick
        (160, 0, 160),    # brighter magenta
        (20, 20, 20)      # less dominant background shade
    ]

    for i in range(5):  # 5 warstw fal
        wave_surface = pygame.Surface((WIDTH, HEIGHT * 2), pygame.SRCALPHA)
        freq = 0.005 + i * 0.002
        base_amp = 16 + (5 - i) * 3
        amp = base_amp + math.sin(time_offset * 2 + i) * 14
        speed = 0.03 + i * 0.01  # wolniejsze przesuwanie
...
while running:
    ...
    night_mode = not (dt_time(5, 0) <= current_hour_minute < dt_time(21, 0))

    if time.time() - last_weather_check > 60:  # używamy tej samej zmiennej do sprawdzania raz na minutę
        if not night_mode:
            os.system("echo 192 | sudo tee /sys/class/backlight/*/brightness > /dev/null")
        else:
            os.system("echo 20 | sudo tee /sys/class/backlight/*/brightness > /dev/null")

    if fade_progress >= 1.0:
        prev_second = datetime.now().second

    draw_wave_background(screen, time.time(), night_mode)
    ...
    # Rysowanie godziny z efektem przejścia
    ...
    new_alpha = int(255 * fade_progress)
    old_alpha = int(255 * (1.0 - fade_progress))

    new_surface.set_alpha(new_alpha)
    old_surface.set_alpha(old_alpha)

    rect_old = old_surface.get_rect(center=(CENTER_X, CENTER_Y))
    rect_new = new_surface.get_rect(center=(CENTER_X, CENTER_Y))

    screen.blit(old_surface, rect_old)
    screen.blit(new_surface, rect_new)
    ...
    for i in range(60):
        ...
        base_color = RED_TINT if night_mode else (WHITE if i <= current_second else DARK_GRAY)

        if fade_progress < 1.0:
            # Wygaszanie poprzednich kresek
...
# Zmniejszenie jasności przy wyjściu
